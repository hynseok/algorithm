...
void init(int N, int K){
    ...
    for(int i=0;i<1001;i++) UserMail[i].clear();
}

void sendMail(char subject[], int uID, int cnt, int rIDs[]){
    
    if(visited.find(subject)!=visited.end()){
        int mailNumber = visited[subject];

        for(int i=0;i<cnt;i++){
            if(UserMail[rIDs[i]].size()==BoxN) UserMail[rIDs[i]].pop_back();
            
            UserMail[rIDs[i]].push_front(mailNumber);
        }
    }
    else{
        visited[subject] = MailN;
        char *ptr = strtok(subject," ");
        int Mailidx = 0;
        while(ptr!=NULL){
            if(HashMap.find(ptr)==HashMap.end()){
                HashMap[ptr] = hashN;
                Mails[MailN][Mailidx] = hashN;
                hashN++;
            }
            else{
                int val = HashMap[ptr];
                Mails[MailN][Mailidx] = val;
            }
            Mailidx++;
            ptr = strtok(NULL," ");
        }
        for(int i=0;i<cnt;i++){
            if(UserMail[rIDs[i]].size()==BoxN) UserMail[rIDs[i]].pop_back();
            UserMail[rIDs[i]].push_front(MailN);
        }
        MailN++;
    }
...
int getCount(int uID){
    return UserMail[uID].size();
}
int deleteMail(int uID, char subject[]){
    int ret = 0;
    int mailNumber = visited[subject];
    auto iter = UserMail[uID].begin();
    while(iter!=UserMail[uID].end()){
        if(*iter==mailNumber){
            UserMail[uID].erase(iter++);
            ret++;
        }
        else{
            ++iter;
